...
public class Main {
  ...
  public enum Command {
    NEW_GAME(
        2,
        "Start a new game: NEW_GAME <DIFFICULTY_LEVEL> <TARGET_SCORE>\n"
            + "\tDIFFICULTY_LEVEL: EASY | MEDIUM | HARD\n"
            + "\tTARGET_SCORE: a positive integer (> 0)",
        "What is your name?"),
    PLAY(0, "Play a round"),
  ...
  private static final String COMMAND_PREFIX = "281-pig-game> ";
  ...
  private static void printBanner() {
    // Use StringBuilder to efficiently construct the multi-line ASCII banner.
    StringBuilder banner = new StringBuilder();

    // Add spacing before the banner.
    banner.append(System.lineSeparator());

    // First section of the banner.
    banner
        .append(
    ...
    // Add spacing between banner sections.
    banner.append(System.lineSeparator());

    // Second section of the banner.
    banner
        .append("__________.__           ________                       ")
        .append(System.lineSeparator());
    banner
        .append("\\______   \\__| ____    /  _____/_____    _____   ____  ")
        .append(System.lineSeparator());
    banner
        .append(" |     ___/  |/ ___\\  /   \\  ___\\__  \\  /     \\_/ __ \\ ")
        .append(System.lineSeparator());
    banner
        .append(" |    |   |  / /_/  > \\    \\_\\  \\/ __ \\|  Y Y  \\  ___/ ")
        .append(System.lineSeparator());
    banner
        .append(" |____|   |__\\___  /   \\______  (____  /__|_|  /\\___  >")
        .append(System.lineSeparator());
    banner
        .append("            /_____/           \\/     \\/      \\/     \\/ ")
        .append(System.lineSeparator());

    // Print the completed banner.
  ...
  private boolean processCommand(String input) {
    ...
    switch (command) {
      case NEW_GAME:
        ...
        String targetScoreString = args[2].toUpperCase();
        if (!isInteger(targetScoreString)) {
          MessageCli.INVALID_TARGET_SCORE.printMessage();
          break;
        }

        int targetScore = Integer.parseInt(targetScoreString);
        if (targetScore <= 0) {
          MessageCli.INVALID_TARGET_SCORE.printMessage();
          break;
        }
        ...
        game.newGame(Difficulty.valueOf(difficulty), targetScore, processOptions(command));
        break;
      case PLAY:
        game.play();
        break;
